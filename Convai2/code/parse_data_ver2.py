...
def split_data(data_file):
    ...
    for line_number, line in enumerate(lines):
        ...
        if len(line_list) == 1:
            ...
            eps['dialog'][0].append(line_list[0])
            eps['dialog'][1].append(line_list[1])
            if len(line_list) == 4:
...
def build_vocab(episodes, embedding_file=None, dim_in_file=None, train_oov=True, count_threshold=0):
    ...
    for eps in episodes:
        ...
        count_vocab(eps['dialog'][0], count_dict)
    ...
    index2vocab = ["<PAD>", "<START>", "<EOS>", "<UNK>", "__SILENCE__"]
    if embedding_file == None:
        index2vocab = index2vocab + [ w for w in count_dict ]
        embedding_weight, embedding_dim = None, None
    else:
...
def episodes_text2index(episodes, vocab2index):
    for eps in episodes:
        ...
        sentence_list2index(eps['dialog'][0], vocab2index)
...
def get_persona_batch(episodes):
    personas = [ d['persona'][1] for d in episodes ]
    ...
    ret = torch.LongTensor(len(episodes), max_num_sentence, max_num_words)
    ret.fill_(idx_PAD)
...
def get_turn_batch(episodes, ith_turn, person_id):
    max_num_words = 0
    num = 0
    for eps in episodes:
        sentence_list = eps['dialog'][person_id]
        if ith_turn < len(sentence_list) and len(sentence_list[ith_turn]) > max_num_words:
            max_num_words = len(sentence_list[ith_turn])
    ret = torch.LongTensor(len(episodes), max_num_words)
    ret.fill_(idx_PAD)
    for i, eps in enumerate(episodes):
        sentence_list = eps['dialog'][person_id]
        if ith_turn < len(sentence_list):
            num += len(sentence_list)
            for j, word_index in enumerate(sentence_list[ith_turn]):
                ret[i, j] = word_index
    return ret, num
def get_dialog_batches(episodes):
    max_num_turn = max([ len(eps['dialog'][0]) for eps in episodes ])
    dialog_batches = []
    for i in range(max_num_turn):
        batch_xs = get_turn_batch(episodes, i, 0)[0]
        batch_ys, num = get_turn_batch(episodes, i, 1) # batch ys length differs, so scale of size average differs, need true num of words
        dialog_batches.append((batch_xs, batch_ys, num))
